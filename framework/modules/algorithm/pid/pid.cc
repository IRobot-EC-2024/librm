...
using modules::algorithm::misc::absConstrain;
...
PID::PID(PIDType type, fp32 kp, fp32 ki, fp32 kd, fp32 max_out, fp32 max_iout, fp32 *external_diff_input) :
    kp_(kp), ki_(ki), kd_(kd), type_(type), max_out_(max_out), max_iout_(max_iout),
    use_external_diff_input_(true), external_diff_input_(external_diff_input) {}
...
void PID::update(fp32 set, fp32 ref) {
  ...
  switch (this->type_) {
    ...
    case PIDType::kPosition:this->p_out_ = this->kp_ * this->error_[0];
      ...
      this->d_buf_[0] = use_external_diff_input_ ?
                        *(this->external_diff_input_) : (this->error_[0] - this->error_[1]);
    ...
    case PIDType::kDelta:this->p_out_ = this->kp_ * (this->error_[0] - this->error_[1]);
      this->i_out_ = this->ki_ * this->error_[0];

      this->d_buf_[2] = this->d_buf_[1];
      this->d_buf_[1] = this->d_buf_[0];
      this->d_buf_[0] = use_external_diff_input_ ?
                        *(this->external_diff_input_) : (this->error_[0] - 2.0f * this->error_[1] + this->error_[2]);
