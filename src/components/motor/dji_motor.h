...
class DjiMotorBase : public irobot_ec::hal::can::CanDeviceBase {
 public:
  ...
  /* 取值函数 */
  [[nodiscard]] u16 encoder() const;
  [[nodiscard]] i16 rpm() const;
  [[nodiscard]] i16 current() const;
  [[nodiscard]] u8 temperature() const;
  /***********/

 protected:
  DjiMotorBase(u8 buffer_index, CAN_HandleTypeDef *hcan, u16 id);

  template <i16 current_bound, u8 buffer_index>
  void SetCurrentTemplate(i16 current);
   ...
   */
  static std::array<std::unordered_map<CAN_HandleTypeDef *, std::array<u8, 18>> *, 3> tx_bufs_;
  std::array<u8, 18> *tx_buffer_p_;  // 指向这个电机发送缓冲区的指针
...
class GM6020 final : public DjiMotorBase {
 public:
  GM6020() = delete;
  GM6020(CAN_HandleTypeDef *hcan, u16 id);
...
class M2006 final : public DjiMotorBase {
 public:
  M2006() = delete;
  M2006(CAN_HandleTypeDef *hcan, u16 id);
...
class M3508 final : public DjiMotorBase {
 public:
  M3508() = delete;
  M3508(CAN_HandleTypeDef *hcan, u16 id);
